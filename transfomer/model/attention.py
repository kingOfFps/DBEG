...
def attention_block(inputs):
    ...
    output_attention_mul = concatenate([inputs, x])
    # output_attention_mul = x
...
def multihead_attention(inputs, num_heads, head_dim):
    """
    多头注意力机制
    :param inputs: 输入张量，shape为(batch_size, time_steps, input_dim)
    :param num_heads: 头数
    :param head_dim: 每个头的维度
    :return: 输出张量，shape为(batch_size, time_steps, num_heads * head_dim)
    """
    input_dim = int(inputs.shape[2])
    query = Dense(num_heads * head_dim)(inputs)
    key = Dense(num_heads * head_dim)(inputs)
    value = Dense(num_heads * head_dim)(inputs)
    query = tf.reshape(query, (-1, int(inputs.shape[1]), num_heads, head_dim))
    key = tf.reshape(key, (-1, int(inputs.shape[1]), num_heads, head_dim))
    value = tf.reshape(value, (-1, int(inputs.shape[1]), num_heads, head_dim))
    query = tf.transpose(query, perm=[0, 2, 1, 3])
    key = tf.transpose(key, perm=[0, 2, 3, 1])
    attention_weights = tf.matmul(query, key) / tf.math.sqrt(float(head_dim))
    attention_weights = tf.nn.softmax(attention_weights, axis=-1)
    output = tf.matmul(attention_weights, value)
    output = tf.transpose(output, perm=[0, 2, 1, 3])
    output = tf.reshape(output, (-1, int(inputs.shape[1]), num_heads * head_dim))
    return output
